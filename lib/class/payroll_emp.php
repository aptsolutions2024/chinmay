...
$this->connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  ...
  public function login($user_name, $pass) {
    ...
    // $sql = "SELECT * FROM employee WHERE user_name = '$user_name' AND role = '5' and job_status in ($_SESSION[active_emp])";
     $sql = "SELECT * FROM employee WHERE user_name = '$user_name' AND role = '5'";
// echo $sql;
...
/*    function showClient1($comp_id,$user_id){
   ...
   function deleteEmpadvances($id){
    ...
    function insertClient($name,$add1,$esicode,$pfcode,$tanno,$panno,$gstno,$mont,$parent,$comp_id,$user_id,$sc,$email,$parent_comp){
        $sql = "INSERT INTO `mast_client`(`client_name`, `client_add1`,`esicode`, `pfcode`, `tanno`, `panno`, `gstno`, `current_month`,parent_comp,`parentId`, `valid_users`,`comp_id`,`email`, `ser_charges`,`db_adddate`, `db_update`) VALUES('".$name."','".$add1."','".$esicode."','".$pfcode."','".$tanno."','".$panno."','".$gstno."','".$mont."','".$parent_comp."','".$parent."','".$user_id."','".$comp_id."','".$email."','".$sc."',NOW(),NOW())";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        return $stmt;
	}
	function updateClient($cid,$name,$add1,$esicode,$pfcode,$tanno,$panno,$gstno,$mont,$parent,$sc,$email,$comp_id,$user_id,$parent_comp){
        $sql = "UPDATE `mast_client` SET `comp_id`='".$comp_id."',`user_id`='".$user_id."',`email`='".$email."',`ser_charges`='".$sc."',`client_name`='".$name."',`client_add1`='".$add1."',`esicode`='".$esicode."',`pfcode`='".$pfcode."',`tanno`='".$tanno."',`panno`='".$panno."',`gstno`='".$gstno."',`current_month`='".$mont."',`parent_comp`='".$parent_comp."',`parentId`='".$parent."',`db_update`=NOW()  WHERE mast_client_id='".$cid."'";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        if ($stmt->rowCount()){
        $msg= 'Success: At least 1 row was affected.';
...
//********************************************************************Salary Menu commaon for attendance and Supervisor and admin ***********************   
   ...
   function displayTranday($id){
      ...
      //************************************************************************
        ...
        $sql = "SELECT emp_id,first_name,middle_name,last_name from `employee` emp WHERE  emp.client_id = '".$client_id."' and job_status in ($_SESSION[inactive_emp]) and emp.emp_id in (SELECT emp_id FROM tran_days)" ;
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $count=$stmt->rowCount();
        ...
        $sql = "SELECT emp_id,first_name,middle_name,last_name from `employee`  emp WHERE  emp.client_id = '".$client."' and emp.job_status in ($_SESSION[active_emp]) and emp.emp_id not in (SELECT emp_id FROM tran_days)" ;
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $count=$stmt->rowCount();
    ...
    function blankExportTranDays($comp_id,$client_id){
     $sql ="select td_string from mast_company where comp_id = '".$comp_id."'";
        ...
        for($i=1; $i<$j; $i++ ){
         ...
         $sql .= ",emp.ticket_no,md.mast_dept_name as 'Dept_Name',emp.comp_ticket_no FROM `employee` emp inner join mast_client mc  on emp.client_id = mc.mast_client_id  inner join mast_dept md on emp.dept_id= md.mast_Dept_id where emp.client_id = '".$client_id."' and emp.job_status in ($_SESSION[active_emp]) order by emp.emp_id";	
        $stmt = $this->connection->prepare($sql);
        //echo $sql;exit;
        $stmt->execute();
        $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
  ...
  function showEployeedetailsQ($id){
   ...
   function getEmployeeDetailsByClientIdAppont($clientid,$cmonth){
    ...
    function getEmployeeIncome($empid){		
	  ...
	  function getEmpTranIncome($tab_emp,$tab_empinc,$comp_id,$tran_day_emp_id,$sal_month){
	       $sql = "select ti.*,mi.income_heads_name from $tab_empinc ti inner join mast_income_heads mi on ti.head_id = mi.mast_income_heads_id inner join $tab_emp  te on te.emp_id =ti.emp_id and te.sal_month = ti.sal_month where mi.comp_id = '$comp_id'  and ti.emp_id = '$tran_day_emp_id' and  ti.sal_month = '$sal_month'  order by mi.mast_income_heads_id";
	       $stmt = $this->connection->prepare($sql);
           $stmt->execute();
           $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
           return $row;
   }
	  function getEmpTranDeduct($tab_emp,$tab_empded,$comp_id,$tran_day_emp_id,$sal_month){
        ...
        function getEmpTranAdvance($tab_adv,$comp_id,$tran_day_emp_id,$sal_month){
        ...
        function getEmpTranDays($tab_days,$emp_id,$sal_month)
        {
            $sql = "select sal_month, `present`, `absent`, `weeklyoff`, `pl`, `sl`, `cl`, `otherleave`, `paidholiday`, `additional`, `othours`, `nightshifts`,`extra_inc2` from $tab_days where emp_id='$emp_id' and sal_month = '$sal_month' ";
           $stmt = $this->connection->prepare($sql);
           $stmt->execute();
           $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
           return $row;
    ...
    function deductEmpData($destid,$clientid){
          $sql = "SELECT emp_de.`std_amt`,emp_de.`calc_type`,emp_de.`remark`,emp_de.emp_deduct_id,emp.first_name as fn,emp.middle_name as mn,emp.last_name as ln FROM `employee` emp,emp_deduct emp_de where emp_de.head_id='".$destid."' AND emp_de.emp_id=emp.emp_id AND emp.client_id='".$clientid."' AND emp.job_status in ($_SESSION[active_emp]) order by emp.first_name,emp.last_name";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        $row = $stmt->fetchAll();
        return $row;
    ...
    function getEmpData($clientid,$fielda,$fieldb,$fieldc,$fieldd){
         $sql = "SELECT $fielda,$fieldb,$fieldc,$fieldd,emp_id,first_name as fn,middle_name as mn,last_name as ln FROM `employee` where client_id='".$clientid."' AND job_status in ($_SESSION[active_emp]) order by emp_id"; 
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        $row = $stmt->fetchAll();
        return $row;
      ...
      function getEmpDataFromTo($clientid,$fielda,$fieldb,$fieldc,$fieldd,$limitfrom,$limitto){
         $sql = "SELECT $fielda,$fieldb,$fieldc,$fieldd,emp_id,first_name as fn,middle_name as mn,last_name as ln FROM `employee` where client_id='".$clientid."' AND job_status in ($_SESSION[active_emp]) order by emp_id limit $limitto offset $limitfrom"; 
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        $row = $stmt->fetchAll();
        return $row;
 ...
 function insertCSVWiseEmployee(
    $comp_id, $user_id, $emapData0, $emapData1, $emapData2, $emapData3, $empdata4, $empdata5, $empdata6, $empdata7, $empdata8, 
    $emapData9, $emapData11, $dept_id,$category_id, $qualif_id, $emapData14, $emapData15, $bank_id, $emapData17, $emapData18, $emapData19, 
    $emapData20, $emapData21, $emapData22, $emapData23, $uname, $password, $desg_id, $clientid
) {
    // SQL query
   $sql = "INSERT INTO `employee` (
       `comp_id`, `user_id`, `first_name`, `middle_name`, `last_name`, `user_name`, `password`,
    ...
    `adharno`, `uan`, `married_status`, `emp_add1`, `desg_id`, `client_id`
) VALUES (
        '$comp_id', '$user_id', '$emapData0', '$emapData1', '$emapData2', '$uname', '$password',
        '$emapData3', '$empdata4', '$empdata5', '$empdata6', '$empdata7', '$empdata8', '$emapData9', 'Y', '$emapData11',
        '$dept_id','$category_id', '$qualif_id', '$emapData14', '$emapData15', '$bank_id', '$emapData17', '$emapData18', '$emapData19',
        '$emapData20', '$emapData21', '$emapData22', '$emapData23', '$desg_id', '$clientid'
    )";
    ...
    function getIncomeEmpDetails_import($comp_id,$user_id,$clint_id){
        $setSql= "SELECT emp_id as 'Employee ID',`first_name` as 'First Name',`middle_name` as 'Middle Name',`last_name` as 'Last Name','0','0' FROM `employee` WHERE  `comp_id`='".$comp_id."' AND`user_id`='".$user_id."' AND employee.client_id='".$clint_id."' and employee.job_status in ($_SESSION[active_emp]) order by first_name,last_name";
        //$setSql= " SELECT distinct emp_id as 'Employee ID',`last_name` as 'Last Name',`first_name` as 'First Name',`middle_name` as 'Middle Name' FROM `employee` WHERE  `comp_id`='".$comp_id."' AND`user_id`='".$user_id."' AND employee.client_id='".$clint_id."' and employee.job_status != 'L' ";
        $stmt = $this->connection->prepare($setSql);
        $stmt->execute();
        $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    function updateEmpIncome($emapData4,$ct,$emapData5,$emapData0,$incomeid,$comp_id,$user_id,$countch){
        if($countch>0) {
            $sql = "update emp_income set std_amt = '$emapData4',`calc_type`='$ct',remark='$emapData5' where `emp_id`='$emapData0' AND `head_id`='$incomeid' ";
        }else{								
            $sql = "INSERT INTO `emp_income`(`comp_id`, `user_id`,`emp_id`,`head_id`, `calc_type`, `std_amt`, `remark`, `db_addate`, `db_update`) VALUES ('$comp_id','$user_id','$emapData0','$incomeid','$ct','$emapData4','$emapData5',Now(),Now())";
        }
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        $row = $stmt->rowCount();
    ...
    function countEmpDeduct($emapData0,$deductid){
        $setSql= "SELECT * FROM `emp_deduct` WHERE `emp_id`='".$emapData0."' AND `head_id`='".$incomeid."' ";
        $stmt = $this->connection->prepare($setSql);
        $stmt->execute();
        $row = $stmt->rowCount();
        return $row;
    ...
    function getEmpByCompId($comp_id,$user_id,$clint_id){
        $sql = "select 'Employee_ID','Last Name','First Name','Middle Name','Std_amt','Remark' union all SELECT distinct emp_id as 'Employee ID',`last_name` as 'Last Name',`first_name` as 'First Name',`middle_name` as 'Middle Name','0','0' FROM `employee` WHERE  `comp_id`='".$comp_id."' AND employee.client_id='".$clint_id."' and employee.job_status in ($_SESSION[active_emp])";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        $row = $stmt->fetchAll();
        return $row;
    ...
    { 
	     $sql = "SELECT e.emp_id,e.first_name as fn,e.middle_name as mn,e.last_name as ln,".$selname.",".$prid.",bankacno FROM employee e inner join $table on e.".$prid."=".$table.".".$tableprid."  where e.job_status in ($_SESSION[active_emp]) and  e.comp_id='".$comp_id."' and client_id = '$cl_id' order by e.emp_id"; 
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
		$row = $stmt->fetchAll();
		return $row;
 ...
 function getClientChild($client_id){
    ...
    { 
	    $sql = "select * from employee where passcode ='$passcode' and mobile_no='$mobile_no' and job_status in ($_SESSION[active_emp])"; 
	    $stmt = $this->connection->prepare($sql);
	    $stmt->execute();
...
function updateempPwdbyunamepass($username, $userpass, $newuserpass) {
    try {
        ...
        $stmt->bindParam(':newuserpass', $newuserpass);
        $result = $stmt->execute();
   ...
   //Added on 28 Aug 2024 by Asharani
     function insertEmployeeNew($fname,$mname,$lname,$uname,$password,$add1,$client,$pin_code,$comp_id,$user_id){
         $passcode=substr(number_format(time() * rand(),0,'',''),0,6); 
         $sql = "INSERT INTO `employee`(`first_name`, `middle_name`, `last_name`, `user_name` ,`password`, `client_id`,`emp_add1`,`pin_code`,`comp_id`,`user_id`,`passcode`,`db_adddate`, `db_update`) VALUES ('".$fname."','".$mname."','".$lname."','".$uname."','".$password."','".$client."','".$add1."','".$pin_code."','".$comp_id."','".$user_id."','".$passcode."',NOW(),NOW())"; 
         $stmt = $this->connection->prepare($sql);
         $stmt->execute();
         return $inserted = $this->connection->lastInsertId();
    }
...
function deleteGroupClient($id) {
  ...
  /* added on 29-08-2024 Datewise details -use for display client as client Id  Starts  */
    function displayemployeeClientbyID($cid){
      $sql = "select * from employee WHERE client_id='".$cid."' AND job_status in ($_SESSION[active_emp]) order by emp_id, first_name,middle_name,last_name";
   ...
   function exportTDDEmpData($emp_id,$sal_month){
	  ...
	  echo "<br>". $sql = "select * from tran_days_details where emp_id ='".$emp_id."' AND sal_month='".$sal_month."' "; 
	 ...
	 function insertUpdateNewEmpTranDaysDetails($sql){ 
      ...
      $sql = "select emp_id from employee WHERE client_id='".$cid."' AND job_status in ($_SESSION[active_emp]) order by emp_id";
       $stmt = $this->connection->prepare($sql);
       $stmt->execute();
       $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
       return $row;
...
public function isUsernameExists($uname) {
    try {
      ...
      echo  $query = "SELECT COUNT(*) as cnt FROM employee WHERE user_name = '$uname'";
...
function displayCategory($did){
    ...
    function getCategorytId($mast_category_name,$comp_id){
       echo $sql = "select mast_category_id from mast_category WHERE mast_category_name Like '%".$mast_category_name."%' and comp_id = '".$comp_id."'  ";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
...
// added by sakshi on 07-11-2024
    public function changPassword($uname, $new_hashed_pass) {
        $query = "UPDATE employee SET password = :new_pass WHERE user_name = :uname";
        $stmt = $this->connection->prepare($query);
   ...
   public function checkUsernameExists($uname){
...
public function checkPassword($user, $old_pass){
    // Verify if the provided password matches the stored password hash
    $query = "SELECT * FROM employee WHERE password = '$old_pass' AND user_name = '$user'";
    echo $query;
    $stmt = $this->connection->prepare($query);
   $stmt->execute();
