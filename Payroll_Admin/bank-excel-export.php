...
if ($clientGrp != '') {
    ...
    $setExcelName = "bank-excelGroup_" . $clientGrp;
...
$row = $payrollAdmin->bank_excel($client_id, $frdt);
...
if (count($row)) {
    foreach ($row as $rec) {
        if (isset($_POST['submit_format1'])) {
            ...
            $setData .= $srNo . "\t" . $rec['bankacno'] . "\tC\tSB\t" . $rec['empname'] . "\t" . $rec['netsalary'] . "\n";
            ...
            $setData .= $srNo . "\tC\t" . $rec['netsalary'] . "\t" . $rec['ifsc_code'] . "\t" . $rec['bankacno'] . "\t" . $rec['empname'] . "\n"; // Adjusted data format for Format 2
            $totalAmount += $rec['netsalary'];
            $setData = str_replace("\r", "", $setData);
        }
