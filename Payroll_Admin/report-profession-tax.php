...
<script type="text/javascript" src="Payroll/js/jquery.min.js"></script>
    ...
    document.getElementById('current-month').textContent = currentMonth || ' '; // Fallback to '--select--' if no month is found
}
        $( function() {
            $("#frdt").datepicker({
                changeMonth: true,
...
<div class="twelve mobicenter innerbg">
   ...
   <!-- Client Dropdown -->
        ...
        <div class="three columns">
            <select class="textclass" name="client" id="client" onchange="updateCurrentMonth(); $('#linkshow').hide(); clientGroupReset()" style="width: 100%;">
...
<script>
    ...
    function saveFormData() {
        ...
        var clientGroup = document.getElementById('clientGroup').value;
        var frdt = document.getElementById('frdt').value;
        var month = document.querySelector('input[name="month"]:checked').value;

        localStorage.setItem('client', client);
        localStorage.setItem('clientGroup', clientGroup);
        localStorage.setItem('frdt', frdt);
        localStorage.setItem('month', month);
    }

    // Call saveFormData when 'Cancel' button is clicked
    function cancelAndSave() {
        saveFormData();
        history.go(-1); // Go back to the previous page
    }

    // Function to restore form data from localStorage on page load
    function restoreFormData() {
        var client = localStorage.getItem('client');
        var clientGroup = localStorage.getItem('clientGroup');
        var frdt = localStorage.getItem('frdt');
        var month = localStorage.getItem('month');

        if (client) document.getElementById('client').value = client;
        if (clientGroup) document.getElementById('clientGroup').value = clientGroup;
        if (frdt) document.getElementById('frdt').value = frdt;

        if (month) {
            document.querySelector(`input[name="month"][value="${month}"]`).checked = true;
            showdates();  // Call to showdates() function to handle showing/hiding dates
        }
    }
